...
const InviteCodeGenerator = ({ onCodeGenerated }) => {
    ...
    const handleGenerate = async () => {
        setLoading(true);
        try {
            const { code, error } = await inviteService.generateInviteCode(user.business_id, role);
            if (error) throw error;

            setGeneratedCode({ code, role }); // inviteService returns object with code
            if (onCodeGenerated) onCodeGenerated({ code, role, status: 'active', created_at: new Date() });
