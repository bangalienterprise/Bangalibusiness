...
const StockValuationCard = ({ costValue, sellingValue, profit, marginPercent, totalItems, isLoading, error }) => {
    if (isLoading) return <SkeletonLoader variant="card" count={4} />;
    
    if (error) return <div className="text-red-500 text-sm mb-4">Error loading valuation: {error}</div>;
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <MetricCard 
                ...
                value={`৳${costValue?.toLocaleString() || 0}`} 
                icon={DollarSign} 
            />
            <MetricCard 
                title="Potential Revenue" 
                value={`৳${sellingValue?.toLocaleString() || 0}`} 
                icon={TrendingUp} 
                colorClass="text-green-400"
            />
            <MetricCard 
                title="Est. Profit Margin" 
                value={`${marginPercent?.toFixed(1) || 0}%`} 
                subtext={`Potential Profit: ৳${profit?.toLocaleString() || 0}`}
                icon={Percent} 
